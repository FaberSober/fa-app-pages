import React, { useContext, useState } from 'react';
import { get } from 'lodash';
import { Button, Form, Input } from 'antd';
import { EditOutlined, PlusOutlined } from "@ant-design/icons";
import { DragModal, FaHref, ApiEffectLayoutContext, FaUtils, CommonModalProps } from '@fa/ui';
import { apkVersionApi as api } from '@/services';
import { App } from '@/types';


/**
 * APP-APK版本表实体新增、编辑弹框
 */
export default function ApkVersionModal({ children, title, record, fetchFinish, addBtn, editBtn, ...props }: CommonModalProps<App.ApkVersion>) {
  const {loadingEffect} = useContext(ApiEffectLayoutContext)
  const [form] = Form.useForm();

  const [open, setOpen] = useState(false);

  /** 新增Item */
  function invokeInsertTask(params: any) {
    api.save(params).then((res) => {
      FaUtils.showResponse(res, '新增APP-APK版本表');
      setOpen(false);
      if (fetchFinish) fetchFinish();
    })
  }

  /** 更新Item */
  function invokeUpdateTask(params: any) {
    api.update(params.id, params).then((res) => {
      FaUtils.showResponse(res, '更新APP-APK版本表');
      setOpen(false);
      if (fetchFinish) fetchFinish();
    })
  }

  /** 提交表单 */
  function onFinish(fieldsValue: any) {
    const values = {
      ...fieldsValue,
      // birthday: FaUtils.getDateStr000(fieldsValue.birthday),
    };
    if (record) {
      invokeUpdateTask({ ...record, ...values });
    } else {
      invokeInsertTask({ ...values });
    }
  }

  function getInitialValues() {
    return {
      appId: get(record, 'appId'),
      name: get(record, 'name'),
      applicationId: get(record, 'applicationId'),
      versionCode: get(record, 'versionCode'),
      versionName: get(record, 'versionName'),
      iconId: get(record, 'iconId'),
      fileId: get(record, 'fileId'),
      size: get(record, 'size'),
      downloadNum: get(record, 'downloadNum'),
      remark: get(record, 'remark'),
      // birthday: FaUtils.getInitialKeyTimeValue(record, 'birthday'),
    }
  }

  function showModal() {
    setOpen(true)
    form.setFieldsValue(getInitialValues())
  }

  const loading = loadingEffect[api.getUrl('save')] || loadingEffect[api.getUrl('update')];
  return (
    <span>
      <span onClick={showModal}>
        {children}
        {addBtn && <Button icon={<PlusOutlined />} type="primary">新增</Button>}
        {editBtn && <FaHref icon={<EditOutlined />} text="编辑" />}
      </span>
      <DragModal
        title={title}
        open={open}
        onOk={() => form.submit()}
        confirmLoading={loading}
        onCancel={() => setOpen(false)}
        width={700}
        {...props}
      >
        <Form form={form} onFinish={onFinish}>
          <Form.Item name="name" label="应用名称" rules={[{ required: true }]} {...FaUtils.formItemFullLayout}>
            <Input placeholder="请输入应用名称" disabled />
          </Form.Item>
          <Form.Item name="applicationId" label="应用包名" rules={[{ required: true }]} {...FaUtils.formItemFullLayout}>
            <Input placeholder="请输入应用包名" disabled />
          </Form.Item>
          <Form.Item name="versionCode" label="版本号" rules={[{ required: true }]} {...FaUtils.formItemFullLayout}>
            <Input placeholder="请输入版本号" disabled />
          </Form.Item>
          <Form.Item name="versionName" label="版本名称" rules={[{ required: true }]} {...FaUtils.formItemFullLayout}>
            <Input placeholder="请输入版本名称" disabled />
          </Form.Item>
          <Form.Item name="remark" label="版本信息" rules={[{ required: true }]} {...FaUtils.formItemFullLayout}>
            <Input.TextArea autoSize placeholder="请输入版本信息" />
          </Form.Item>
        </Form>
      </DragModal>
    </span>
  )
}
